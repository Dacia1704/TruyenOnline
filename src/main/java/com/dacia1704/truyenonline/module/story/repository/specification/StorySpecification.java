...
import com.dacia1704.truyenonline.module.story.dto.request.StoryAdminFilter;
...
public class StorySpecification {
            ...
            if (filters.getUploaderId() != null) {
                predicates.add(
                        criteriaBuilder.equal(
                                root.get("uploader").get("id"), filters.getUploaderId()));
            }
            ...
            if (filters.getSortType() != null && query != null) {
                switch (filters.getSortType()) {
                    ...
                    case FOLLOW -> {
                        ...
                        query.orderBy(criteriaBuilder.desc(criteriaBuilder.count(bookmarkJoin)));
                    }
                }
            } else if (query != null) {
    ...
    public static Specification<Story> filterStories(StoryAdminFilter filters) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();
            ...
            // 4. Lọc (Filter) theo trạng thái ban
            if (filters.getIsBanned() != null) {
                predicates.add(criteriaBuilder.equal(root.get("isBanned"), filters.getIsBanned()));
            }
